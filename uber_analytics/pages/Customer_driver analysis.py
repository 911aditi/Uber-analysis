...
@st.cache_data
def load_data():
    df = pd.read_csv(r"C:\Users\imadi\OneDrive\Desktop\python\cleaned_data.csv")
...
st.markdown("<p style='color:brown; text-align: left; font-size:18px; font-weight:bold'>Booking Value vs Customer Rating: </p>", unsafe_allow_html=True)
df_clean = df_filtered[df_filtered['Customer Rating']!=0].copy()
fig = px.scatter(df_clean,x='Booking Value',y='Customer Rating')
st.plotly_chart(fig)
st.markdown("##### CONCLUSION📊")
st.markdown("""
            - Checks if higher fares impact customer satisfaction.
            - This shows that there is no correlation between booking value and Customer ratings.""")

st.markdown("<p style='color:brown; text-align: left; font-size:18px; font-weight:bold'>Booking Value vs Driver Rating: </p>", unsafe_allow_html=True)
df_clean = df_filtered[df_filtered['Driver Ratings']!=0].copy()
fig = px.scatter(df_clean,x='Booking Value',y='Driver Ratings')
st.plotly_chart(fig)
st.markdown("##### CONCLUSION📊")
st.markdown("""
            - Checks if high-value rides affect driver satisfaction.
            - Driver ratings is also not influenced by booking value.""")
